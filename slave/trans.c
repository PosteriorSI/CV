...
void InitGuestNodeInfo(void)
{
	int size;

	size=sizeof(int)*NODENUM;
	guestNodeInfo=(int*)malloc(size);

	memset(guestNodeInfo, 0, size);
}
...
bool CommitTransactionbak(void)
{
	...
	for(i=0;i<NODENUM;i++)
	{
		if(nodeinfo[i] > 0)
		{
			...
			*(sbuffer)=cmd_localPrepare;
			...
			sbuffer32=(uint32_t*)(sbuffer+1);

			for(j=0;j<NODENUM*THREADNUM;j++)
			{
				*(sbuffer32+j)=*InvisibleTableLocate(j, lindex);
			}

			num=1+(NODENUM*THREADNUM+1)/2;
         ...
         recv(conn, rbuffer, num*sizeof(uint32_t), 0);
			...
			*/
			...
			MergeReadList(((uint32_t*)rbuffer)+1);
		}
	}
	...
	if(commit)
	{
		...
		//WriteCollusion.
		WriteCollusionbak(tid, index);

		cid=0;
		//to do here, commit directly.
		for(i=0;i<NODENUM;i++)
		{
			if(nodeinfo[i] > 0)
			{
...
void ServiceLocalPrepare(int conn, uint64_t* buffer)
{
	...
	InvisibleArray=(uint32_t*)(buffer+1);
	td->InvisibleArray=(TransactionId*)InvisibleArray;
	...
	for(i=0;i<NODENUM*THREADNUM;i++)
		*(((uint32_t*)sbuffer)+1+i)=ReadList[i];
	num=1+NODENUM*THREADNUM;
