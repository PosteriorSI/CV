...
bool MVCCVisiblebak(Record * r, VersionId v)
{
	TransactionData* td;
	TransactionId tid, InvisibleTid;;
	int self_index, w_index;
	TransactionId* InvisibleArray;

	td=(TransactionData*)pthread_getspecific(TransactionDataKey);
	InvisibleArray=td->InvisibleArray;
	
	tid=r->VersionList[v].tid;
	w_index=(tid-1)/MaxTransId;

	InvisibleTid=(TransactionId)(*(InvisibleArray+w_index));

	//if(InvisibleTid <= tid)
	if ((InvisibleTid > InvalidTransactionId) && InvisibleTid <= tid)
		return false;
	else
		return true;
}
...
bool IsUpdateConflictbak(Record * r, TransactionId tid)
{
	//self already updated the data, note that rear is not the newest version.
	VersionId newest;
	...
	else
	{
		...
		if(MVCCVisiblebak(r, r->lcommit) && !IsMVCCDeleted(r, r->lcommit))
			return true;
		else
			return false;
	}
...
void ProcessCollusionInsert(uint64_t * recv_buffer, int conn, int sindex)
{
	...
	if(proc->tid == tid)
	{
		//to optimize, if proc->complete > 0, status can also be set to 0.
		if(IsRead)
		{
			//sid=proc->sid_min;
